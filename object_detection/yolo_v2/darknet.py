...
def _process_batch(data, size_index):
    ...
    for i, cell_ind in enumerate(cell_inds): # 最后只会标注 gt_bbox所在的 cell, 和gt_bbox选出的最佳 anchor
...
class Darknet19(nn.Module):
    def __init__(self):
        ...
        # darknet
        self.conv1s, c1 = _make_layers(3, net_cfgs[0:5]) # input_channels, the parameters of network
        ...
        # cat [conv1s, conv3]
        self.conv4, c4 = _make_layers((c1*(stride*stride) + c3), net_cfgs[7]) # in_channels = 512 * 4 + 1024 = 3072
        # conv4 = 13 * 13 * 1024, c4 = 1024

        # linear
        out_channels = cfg.num_anchors * (cfg.num_classes + 5) # 5 * (20 + 5)
        self.conv5 = net_utils.Conv2d(c4, out_channels, 1, 1, relu=False) # 13 * 13 * out_channels
    ...
    def forward(self, im_data, gt_boxes=None, gt_classes=None, dontcare=None,
        ...
        bbox_pred = torch.cat([xy_pred, wh_pred], 3) # 这里是相对于网格点的偏移量
