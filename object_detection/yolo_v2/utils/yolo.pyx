...
cdef yolo_to_bbox_c(
    ...
    anchors: (num_anchors, 2) (pw, ph) # 一个代表高度，一个代表宽度
    Returns
    -------
        ...
        for row in range(H):
            for col in range(W):
                ...
                for a in range(num_anchors):
                    cx = (bbox_pred[b, ind, a, 0] + col) / W # bx = sig(tx) + cx
                    cy = (bbox_pred[b, ind, a, 1] + row) / H # by = sig(ty) + cy
                    bw = bbox_pred[b, ind, a, 2] * anchors[a][0] / W * 0.5 # bw = exp(tw) * pw
                    bh = bbox_pred[b, ind, a, 3] * anchors[a][1] / H * 0.5 # bh = exp(th) * ph
