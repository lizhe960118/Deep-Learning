...
class AnchorTargetCreator(object):
    ...
    def __call__(self, bbox, anchor, img_size):
        ...
        * :math:`R` is the number of bounding boxes.
        ...
        # argmax_ous:与每个锚框算得最大IOU的实际框的索引 （ < K*A）
        # label：每个锚框的预分类 （ < K * A)

        # compute bounding box regression targets
        loc = bbox2loc(anchor, bbox[argmax_ious]) # (< K*A, 4)

        # map up to original set of anchors
        label = _unmap(label, n_anchor, inside_index, fill=-1) # 恢复到所有框的范围
    ...
    def _create_label(self, inside_index, anchor, bbox):
        ...
        # argmax_ious：（len(inside_index))
        # max_ious (len(inside_index)) # 内容是当前锚框和所有bbox计算得到的最大IOU
        # gt_argmax_ious:(n_bbox) 内容是（0 - len(inside_index)-1)

        # assign negative labels first so that positive labels can clobber them
        label[max_ious < self.neg_iou_thresh] = 0

        # positive label: for each gt, anchor with highest iou
        # 将所有与某一bbox计算有最大IOU的锚框所在的位置标为正类
        label[gt_argmax_ious] = 1 # 即使他们小于负类阈值
        ...
        if len(pos_index) > n_pos:
            disable_index = np.random.choice(
                pos_index, size=(len(pos_index) - n_pos), replace=False) # 选出不需要的置为-1
...
def _unmap(data, count, index, fill=0):
    ...
    if len(data.shape) == 1: # 对应data==label
        ret = np.empty((count,), dtype=data.dtype)
        ret.fill(fill)
        ret[index] = data
    else:
        ret = np.empty((count,) + data.shape[1:], dtype=data.dtype) # 定义返回的值的大小，count是多少行，data.shape[1:]是多少列
        ret.fill(fill)
        ret[index, :] = data
    return ret
...
class ProposalCreator:
    ...
    def __call__(self, loc, score,
        ...
        # anchor (K * A)
        roi = loc2bbox(anchor, loc) # 由锚框和偏移量计算出新的框位置
        ...
        min_size = self.min_size * scale # 16, scale can be small when detect small object 
        hs = roi[:, 2] - roi[:, 0]
        ws = roi[:, 3] - roi[:, 1]
