...
# 普通卷积使用 out_channels 个 3 * 3 * in_channels 卷积提取特征
# 参数大小 out_channels * 3 * 3 * in_channels 

# 可分离卷积先使用 in_channels 个3 * 3 * 1分别卷积每一个channel, 
# 得到和原来大小相同但是经过特征提取的特征图
# 再使用 out_channels 个 1 * 1 * in_channel卷积进行融合
# 参数大小 in_channels * 3 * 3 + out_channels * in_channels
class SeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, dw_kernel_size, dw_stride, dw_padding):
        super(SeparableConv2d, self).__init__()
        self.depthwise_conv2d = nn.Conv2d(in_channels, in_channels, dw_kernel_size, stride=dw_stride, padding=dw_padding, groups=in_channels)
        # groups = in_channels 实现 通道分离
        self.pointwiseConv2d = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1)
...
class BranchSeparables(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=False):
        ...
        self.relu_2 = nn.ReLU()
        self.separable_2 = SeparableConv2d(in_channels, out_channels, kernel_size, stride, padding, bias=bias)
        self.bn_sep_2 = nn.BatchNorm2d(out_channels, eps=0.001, momentum=0.1, affine=True)
    
    def forward(self, x):
        out = self.relu_1(x)
        out = self.separable_1(out)
        out = self.bn_sep_1(out)
        out = self.relu_2(out)
        out = self.separable_2(out)
        out = self.bn_sep_2(out)
        return out 
...
class BranchSeparablesStem(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=False):
        ...
        self.relu_2 = nn.ReLU()
        self.separable_2 = SeparableConv2d(out_channels, out_channels, kernel_size, stride, padding, bias=bias)
        self.bn_sep_2 = nn.BatchNorm2d(out_channels, eps=0.001, momentum=0.1, affine=True)
...
class BranchSeparablesReduction(BranchSeparables):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding,  z_padding=1, bias=False):
        BranchSeparables.__init__(self, in_channels, out_channels, kernel_size, stride, padding)
