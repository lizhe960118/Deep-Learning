...
classify(rotated_image, correct_class=img_class, target_class=demo_target)
...
for i in range(num_examples):
	...
	average_loss += tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=labels) /num_examples

optim_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(average_loss, var_list=[x_hat])

demo_epsilon = 8.0/255.0
demo_lr = 2e-1
demo_steps = 300
demo_target = 924
...
for i in range(demo_steps):
	_, loss_value = sess.run([optim_step, average_loss], feed_dict={learning_rate:demo_lr, y_hat:demo_target})
	if (i+1) % 50 == 0:
		print('step: %d, loss=%g' % (i+1, average_loss))
